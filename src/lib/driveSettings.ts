...
export async function saveDriveFolder(
  ...
  window.dispatchEvent(new Event("baudoku:drive-settings"));
  let timer: ReturnType<typeof setTimeout>;
  try {
    await Promise.race([
      setDoc(
...
export async function getRootFolder(token: string): Promise<string> {
  ...
  assertOwner(owner);
  await saveDriveFolder({ id, name: "Baudokumentationen (App)" });
  assertOwner(owner);
  return id;
}
...
export async function chooseDriveFolder(
  ...
  return new Promise((resolve, reject) => {
    ...
    new picker.PickerBuilder()
      .setTitle("Speicherordner für Baudoku wählen")
      .addView(view)
      .setOAuthToken(token)
