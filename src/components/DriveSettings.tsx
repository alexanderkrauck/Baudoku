...
export default function DriveSettings({
  ...
  async function select(useDefault = false) {
    ...
    try {
      ...
      if (selected) {
        ...
        setMessage(
          warning ||
            "Speicherort gespeichert. Neue Begehungen werden hier abgelegt.",
        );
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <p className="muted">
        Audio, Fotos und Berichte liegen in Ihrem Drive. Jede Begehung erhält
      ...
      >
        ...
        <strong>{folder?.name || "Baudokumentationen (App)"}</strong>
        {folder && (
          <a
      ...
      <p className="muted" style={{ fontSize: ".85rem" }}>
        Eine Änderung gilt für neue Begehungen. Bestehende Berichte bleiben in
